...
export class EnvironmentRenderer{
  ...
  update(state:EnvironmentState,dt:number){this.lastState=state;const daylight=state.solar.daylight,night=1-daylight,cloud=state.weather.cloudCover,sunset=Math.max(0,1-Math.abs(state.solar.elevationDeg)/8)*(1-cloud*.6);
    this.uniforms.daylight.value=daylight;this.uniforms.night.value=night;this.uniforms.wetness.value=state.wetness;this.uniforms.snow.value=state.snowCoverage;this.uniforms.windowLight.value=.035+.18*night;this.uniforms.wind.value.set(state.windEastMps,state.windNorthMps);
