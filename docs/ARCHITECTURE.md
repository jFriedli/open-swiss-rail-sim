...
The scenery manifest partitions terrain into four sectors with shared edge columns. Each sector maps a content-hashed SWISSIMAGE WebP texture directly from its LV95-derived bounds. This manifest abstraction is also the integration point for water, buildings, vegetation and roads. Texture URLs are resolved relative to the manifest, which keeps repository-subpath Pages deployment correct.
