...
| Direction detected by the word `Rapperswil` | AI simulation | explicit path orientation (compiler migration pending) |
| Fixed station names and four calls | journey/traffic scripts and smoke tests | compiler must derive GTFS calls; runtime journey arrays are already variable length |
| Fixed trip, date and 30-minute window | journey/traffic scripts | compiler configuration and derived trip window |
...
| Tests name Blumenau/Uznach and fixed chainages | Playwright | generated package checkpoints and catalogue metadata |
