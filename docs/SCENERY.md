...
Ten official swissBUILDINGS3D CityGML source tiles contribute 7,965 accepted corridor buildings. Semantic LOD2 roof and wall polygons are preserved, grounded as a whole from GroundSurface/terrain comparison, and emitted as four indexed sectors with two material batches each: 473,170 triangles and 12.60 MB. Sector streaming loads the current/ahead area and disposes passed GPU geometry. Roof vertex colours are robust, restrained samples derived from aligned SWISSIMAGE; walls use a stable muted palette because the source has no useful appearance payload. See [BUILDING_ALIGNMENT.md](BUILDING_ALIGNMENT.md).
