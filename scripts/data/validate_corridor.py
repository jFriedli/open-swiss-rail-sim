...
largest_edge=t['stats'].get('largestTriangleEdgeM',t['spacingM']*math.sqrt(2));assert largest_edge<150
max_neighbour=t['stats'].get('maxNeighbourElevationDeltaM',max(abs(t['heights'][r*t['width']+c]-t['heights'][r*t['width']+c+1]) for r in range(t['height']) for c in range(t['width']-1)));assert max_neighbour<50
...
alignment=json.loads(pathlib.Path('data/manifests/rapperswil-uznach-building-alignment.json').read_text());assert building_count==alignment['buildings'] and building_count>5000 and building_triangles>400_000
assert alignment['semanticPolygons']['roof']>0 and alignment['semanticPolygons']['wall']>0 and alignment['corrected']>7000
network=json.loads(p.with_name('rail-network.json').read_text());node_ids={n['id'] for n in network['nodes']};assert len(node_ids)==network['stats']['nodes'] and network['stats']['edges']>50 and network['stats']['switches']>0
...
edge_ids={e['id'] for e in network['edges']};assert all(s['s']>=0 for s in d['signals'])
traffic=json.loads(p.with_name('traffic.json').read_text());assert traffic['stats']['selectedAiServices']>=2 and traffic['stats']['playerTripActiveOnDate'] is True
