...
import argparse,collections,concurrent.futures,csv,datetime,hashlib,heapq,io,json,math,pathlib,random,subprocess,time,urllib.parse,urllib.request,zipfile
...
def compile_route(route_id,definition,force=False,analyse=False,trip_id=None):
    started=time.perf_counter();source=ROOT/f"data/intermediate/candidates/{definition['candidate']}.json";tid,calls,trip,candidate_trips=resolve_trip(definition,trip_id);data,ways,coords,degree,switches,path_nodes,origin_station,destination_station=route_path(definition,source);path_ll=[coords[n] for n in path_nodes];samples=resample(path_ll);origin_e,origin_n=lv95(*samples[0]);route_length=sum(dist(a,b) for a,b in zip(samples,samples[1:]));mapped_signals=[]
    ...
    imagery=out/'imagery';imagery.mkdir(exist_ok=True);tile_columns=max(1,math.ceil((width-1)*spacing/4000));splits=[round(i*(width-1)/tile_columns) for i in range(tile_columns+1)];tiles=[];raw_dir=RAW/'route-compiler'/route_id;raw_dir.mkdir(parents=True,exist_ok=True)
    for i,(c0,c1) in enumerate(zip(splits,splits[1:])):
        ...
        temp=imagery/f'tile-{i}.webp';Image.open(jpg).save(temp,'WEBP',quality=76,method=6);digest=hashlib.sha256(temp.read_bytes()).hexdigest()[:10];final=imagery/f'tile-{i}-{digest}.webp';temp.replace(final);tiles.append({'id':f'scenery-{i}','bounds':{'minX':x0,'maxX':x1,'minZ':z0,'maxZ':z1},'terrain':{'columnStart':c0,'columnEnd':c1},'imagery':{'url':f'./imagery/{final.name}','format':'WebP','width':px,'height':py,'metresPerPixel':round((x1-x0)/px,2),'source':'SWISSIMAGE via official WMS','classification':'REAL'}})
